...
class HERBRobot(Robot):
    ...
    def __init__(self, left_arm_sim, right_arm_sim, right_ft_sim,
        ...
        prpy.bind_subclass(self.left_arm, WAM,
                           sim=left_arm_sim, owd_namespace='/left/owd')
        prpy.bind_subclass(self.right_arm, WAM,
                           sim=right_arm_sim, owd_namespace='/right/owd')
        prpy.bind_subclass(self.head, HERBPantilt,
                           sim=head_sim, owd_namespace='/head/owd')
        ...
        accel_limits[self.left_arm.GetArmIndices()] = [2.] * \
            self.left_arm.GetArmDOF()
        accel_limits[self.right_arm.GetArmIndices()] = [2.] * \
            self.right_arm.GetArmDOF()
        self.SetDOFAccelerationLimits(accel_limits)

        # Support for named configurations.
        import os.path
        self.configurations.add_group(
            'left_arm', self.left_arm.GetArmIndices())
        self.configurations.add_group(
            'right_arm', self.right_arm.GetArmIndices())
        ...
        self.configurations.add_group(
            'right_hand', self.right_hand.GetIndices())

        configurations_path = FindCatkinResource(
            'herbpy', 'config/configurations.yaml')
